...
4. **No DB, no auth, no persistence.** Per the brief; the lesson resets on refresh. State lives in a `useReducer` lesson machine (`lib/lessonMachine.ts`) with explicit `intro → reading → mcq → (remediation → retry)? → simulation → mcq → voiceTutor → done` transitions and 16 unit tests.
