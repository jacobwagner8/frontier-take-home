...
3. **Simulation is one toggle, not a sandbox.** The lesson is about one physical concept (parallel return paths through the EGC). The SVG isolates exactly that: a checkbox adds a second bond at the subpanel, the EGC's current arrows start animating, and a "current now flowing on the EGC" warning appears alongside a caption swap. No load slider, no fault scenarios — those would split attention away from the concept. Mobile-first is also a hard constraint here: most students hit this on a phone between job-site visits or on a break, and the simulation has to land the key concept on a 5-inch screen with one thumb. Deeper exploratory work — meter readings, switch positions, multi-fault scenarios — belongs in the hands-on lab during in-person training. This is the portable concept primer, not a replacement for bench time.

## What I'd build next
1. **Mature the eval engine + author golden content, curricula, and rubrics**, so factual-accuracy, learning-goal coverage, and remediation quality can be scored without a human in the loop. The scaffolding is in `experimental/eval-engine/` (atomic-fact KB, slot extraction, coverage scoring); what's missing is enough golden lessons + reference outputs to calibrate the scorers and per-rubric pass thresholds to fail-fast on regressions. The end state is a generation pipeline that ships only when its output clears the bar automatically.
2. **Broaden factual grounding** — pull in Mike Holt explanatory material and link specific NEC code articles directly into the atomic-fact KB. Today the tutor and follow-up chat can only cite the ten facts in `experimental/eval-engine/kg/atomic-facts/`; with a richer grounded corpus, the chat could cite specific code passages safely instead of hedging or staying generic.
3. **Personalize lesson content to the student** — current knowledge, background, and reading ability. Once the eval engine validates generated content, the same pipeline can produce lessons calibrated for a tested apprentice vs. a journeyman brushing up, or shorten the reading body for a lower-reading-level student without dropping concepts or rigor.
4. **Per-student progress + spaced repetition**, so retention is measured, not just first-pass completion.
5. **A telemetry layer recording which wrong MC options students pick most often**, so the curriculum team can refine remediation paragraphs against real evidence.
